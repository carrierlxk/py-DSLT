...
def run_tracker(p):
    ...
    z_feat1 = net.feat_extraction1(Variable(z_crop_tensor).to(device))
    z_features = net.feat_extraction2(z_feat1)
    ...
    for i in range(0, len(img_list)):
        if i > 0:
            ...
            # get features of search regions 3, 256, 22, 22
            x_crops_tensor = torch.FloatTensor(3,256,22,22)#(x_crops.shape[3], x_crops.shape[2], x_crops.shape[1], x_crops.shape[0])
            ...
            for k in range(x_crops.shape[3]):
                ...
                temp = 255.0 * F.to_tensor(tmp_x_crop).unsqueeze(0)
                temp1, x_crops_tensor[k, :, :, :] = net.forward(Variable(z_crop_tensor.cuda()), Variable(temp.cuda()))
 

            # get features of search regions
            #temp,x_features = net.forward(Variable(z_crop_tensor).to(device), Variable(x_crops_tensor).to(device))
            #print('feature size:', x_crops_tensor.type())
            # evaluate the offline-trained network for exemplar x features.to(device)
            target_position, new_scale = tracker_eval(net, round(s_x), z_features, x_crops_tensor.cuda(), target_position, window, p)
...
if __name__ == "__main__":
    ...
    if demp_type == "single":   # single video demo
        ...
        for video in all_videos:
            ...
            if p.bbox_output:
                np.savetxt(p.bbox_output_path + p.video + '_SiamFC.txt', bbox_result, fmt='%.3f')
            fps_all = fps_all + fps
