...
def run_tracker(p):
    ...
    for i in range(0, len(img_list)):
        if i > 0:
            ...
            #print('feature size:', x_features.size())
...
if __name__ == "__main__":
    ...
    p.net = "SiamFC_60_model.pth"

    # choose demo type, single or all
    demp_type = "all"
    for ii in range(32,50,3): #3520
        p.net = "SiamFC_"+str(ii)+"_model.pth"
        bbox_output_path1 = p.bbox_output_path+str(ii)+"/"
        if demp_type == "single":   # single video demo
            ...
            if p.bbox_output:
                if not os.path.exists(bbox_output_path1):
                    os.makedirs(bbox_output_path1)
            ...
            for video in all_videos:
                ...
                if p.bbox_output:
                    np.savetxt(bbox_output_path1 + p.video + '_SiamFC.txt', bbox_result, fmt='%.3f')
                fps_all = fps_all + fps
