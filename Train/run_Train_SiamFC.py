...
def train(data_dir, train_imdb, val_imdb, model_save_path="./model/", use_gpu=True):
    ...
    for i in range(config.num_epoch):
        ...
        for j, data in enumerate(tqdm(train_loader)):
            ...
            #    np.save('label.npy', train_eltwise_label.detach().cpu().numpy())
        ...
        file.write(str(np.mean(train_loss))+'\n')
        print ("Epoch %d   training loss: %f, validation loss: %f" % (i+1, np.mean(train_loss), np.mean(val_loss)))


if __name__ == "__main__":

    data_dir = "/media/xiankai/Data/tracking/ILSVRC2015_crops/Data/VID/train"
    train_imdb = "/media/xiankai/Data/tracking/SiamFC-PyTorch/ILSVRC15-curation/imdb_video_train.json"
    val_imdb = "/media/xiankai/Data/tracking/SiamFC-PyTorch/ILSVRC15-curation/imdb_video_val.json"
